...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST["wrapProgram2"])) {
		$wrapProgram = $_POST["wrapProgram2"];
	}
	...
	if (!empty(trim($_POST["vehicleGrossWeight"]))) {
		...
		if ($Vehicle_Gross_Weight == "type 1") {
			...
			$Vehicle_Type = 3;
		} else if ($Vehicle_Gross_Weight == "type 4") {
			$Vehicle_Type = 4;
		} else if ($Vehicle_Gross_Weight == "type 5") {
			$Vehicle_Type = 5;
		} else {
	...
	if (isset($_POST["wearables"]) && !empty(trim($_POST["wearables"]))) {
		$wearables_type = trim($_POST["wearables"]);
	} else {
		$wearables_type = "N";
	}
	if (isset($_POST["wrap_program"]) && !empty(trim($_POST["wrap_program"]))) {
		if (trim($_POST["wrap_program"]) == 'Y') {
			...
			if (!empty(trim($_POST["wrapProgram2"]))) {
				...
				if ($Vehicle_Gross_Weight == "type 3") {
					$Vehicle_Type = 3;
					$_POST["vehicleGrossWeight"] = "type 3";
				} elseif ($Vehicle_Gross_Weight == "type 4") {
					$Vehicle_Type = 4;
					$_POST["vehicleGrossWeight"] = "type 4";
				}
			}
		} else {
			$wrap_program = 'N';
	...
	if (isset($_POST["wearables"]) && !empty(trim($_POST["wearables"]))) {
		$wearable_Package = trim($_POST["wearables"]);
	}
	...
	if (isset($_POST["isACP"]) && !empty(trim($_POST["isACP"]))) {
		if ($_POST["isACP"] == "Y") {
			$ACP_Flg = trim($_POST["isACP"]);
		} else {
			$ACP_Flg = 'N';
		}
	} else {
	...
	if (isset($_POST["isHUDS"]) && !empty(trim($_POST["isHUDS"]))) {
		if ($_POST["isHUDS"] == "Y") {
			$HUDS_Flg = trim($_POST["isHUDS"]);
		} else {
			$HUDS_Flg = 'N';
		}
	} else {
	...
	if (isset($_POST["isUCP"]) && !empty(trim($_POST["isUCP"]))) {
		if ($_POST["isUCP"] == "Y") {
			$UCP_Flg = trim($_POST["isUCP"]);
		} else {
			$UCP_Flg = 'N';
		}
	} else {
	...
	if (($warrantyID != "") && $isEdited) {
		...
		$query = "SELECT c.Cntrct_Dim_ID, cd.Cntrct_Lvl_Cd, cd.Cntrct_Term_Mnths_Nbr, c.Veh_ID,
				  cd.AEP_Flg, cd.Aerial_Flg, cd.APU_Flg, cd.wearables_flag, cd.EVBC_Flg, cd.EEC_Flg
		...
		$currentWearable = $row["wearables_flag"];
		$Veh_ID = $row["Veh_ID"];

		$currentAPU = $row["APU_Flg"];
		$currentAerial = $row["Aerial_Flg"];
		$currentAEP = $row["AEP_Flg"];
		$currentEVBC = $row["EVBC_Flg"];
		$currentEEC = $row["EEC_Flg"];
		$currentACP = $row["ACP_Flg"];
		$currentHUDS = $row["HUDS_Flg"];
		$currentUC = $row["UCP_Flg"];
		...
		$currentVehicleYear = (int)$row["Veh_Model_Yr_Cd"];
		...
		if (($currentTier != $Tier_Type) || ($currentTerm != $Coverage_Term) || ($currentCoverage != $Vehicle_Type) ||
			($currentAEP != $Apparatus_Equipment_Package) || ($currentAerial != $Aerial_Package) || ($currentAPU != $APU_Flg) || ($currentEVBC != $EVBC_Flg) || ($currentEEC != $EEC_Flg) || ($currentACP != $ACP_Flg) || ($currentHUDS != $HUDS_Flg) || ($currentUC != $UCP_Flg) ||
			$currentVehicleNewFlag != $Vehicle_New_Flag || $wasOld != $nowIsOld || $currentWearable != $wearable_Package
		) {
		...
		$stmt = mysqli_prepare($link, "UPDATE Cntrct_Dim SET Cstmr_Nme=?,Contract_Date=?,Sply_Pkt_To_Be_Shipd_Flg=?,
									   ...
									   AEP_Flg=?,Aerial_Flg=?,APU_Flg=?,EVBC_Flg=?,EEC_Flg=?,ACP_Flg=?,HUDS_Flg=?,UCP_Flg=?,Small_Goods_Pkg_Flg=?,Cntrct_Term_Mnths_Nbr=?,
									   ...
									   Srvc_Veh_Flg=?,PO_Nbr=?,Wrap_Flg=?,wearables_flag=? WHERE
									   Cntrct_Dim_ID=?");
		...
		mysqli_stmt_bind_param($stmt, "sssssssssssssssdsssissssssissssssi", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $val29, $val30, $val31, $val32, $val33, $val10, $val11, $val12, $val13, $val14, $val15, $val16, $val17, $val18, $val19, $val20, $val21, $val22, $val23, $val24, $val25, $val26, $val27, $wearables_type, $val28);
		...
		if ($smallGoodsPackage == "N") {
			...
			$query = "UPDATE Cntrct SET
				Tot_Sales_Agt_Cost_Amt=(COALESCE(Sales_Agt_Cost_Amt,0)+COALESCE(Sales_Agt_Sml_Goods_Cst_Amt,0)+COALESCE(Addl_Sales_Agt_Cost_Amt,0)),
				Tot_Sales_Agt_Commission_Amt=(COALESCE(Sales_Agt_Commission_Amt,0)+COALESCE(Sales_Agt_Sml_Goods_Commission_Tot_Amt,0)+COALESCE(Addl_Sales_Agt_Commission_Amt,0)),
				Tot_Dlr_Cost_Amt=(COALESCE(Dlr_Cost_Amt,0)+COALESCE(Dlr_Sml_Goods_Cst_Tot_Amt,0)+COALESCE(Addl_Dlr_Cost_Amt,0)),
				Tot_Dlr_Mrkp_Max_Amt=(COALESCE(Dlr_Mrkp_Max_Amt,0)+COALESCE(Dlr_Sml_Goods_Max_Mrkp_Tot_Amt,0)+COALESCE(Addl_Dlr_Mrkp_Max_Amt,0)),";

			$query .= "Tot_Dlr_Mrkp_Act_Amt=(COALESCE(Dlr_Mrkp_Actl_Amt,0)+COALESCE(Dlr_Sml_Goods_Actl_Mrkp_Tot_Amt,0)+COALESCE(Addl_Dlr_Mrkp_Actl_Amt,0)),";

			$query .= "Tot_MSRP_Amt=(COALESCE(MSRP_Amt,0)+COALESCE(Sml_Goods_Tot_Amt,0)+COALESCE(Addl_MSRP_Amt,0)) WHERE Cntrct_ID=" . $warrantyID . ";";
		...
		if($_SESSION['role_ID']=="1" || $_SESSION['admin_loggedin']==true){
			...
			if ($numRows > 0) {
				$ctrctRow = mysqli_fetch_assoc($cntrctResult);
				$wholesale_flg = $ctrctRow["Wholesale_Flg"];
			}else{
				$wholesale_flg = "N";
			}
		}else{
		...
		if ($wrap_program == 'N') {
			$warrantyBasePricingResult = selectWarrantyBasePricing($link, $Coverage_Term, $Vehicle_Type, $Tier_Type, $wholesale_flg);
		} else {
			$warrantyBasePricingResult = selectwrapWarrantyBasePricing($link, $wrap_Program_Term, $Vehicle_Type, $Tier_Type, $wholesale_flg);
		}

		if ($wrap_program == 'N') {
			$newTerm = $Coverage_Term;
		} else {
			$newTerm = $wrap_Program_Term;
		}
		...
		if ($structureChangeFlag == "Y") {
			...
			$stmt = mysqli_prepare($link, "UPDATE Cntrct SET Sales_Agt_Cost_Amt=?, Sales_Agt_Commission_Amt=?,
			                               Dlr_Cost_Amt=?, Dlr_Mrkp_Max_Amt=?,Additional_Commission_Amt=? WHERE
			                               Cntrct_ID=?");
			...
			mysqli_stmt_bind_param($stmt, "iiiiii", $val1, $val2, $val3, $val4, $val8, $val7);
			$result = mysqli_stmt_execute($stmt);
			...
			if (is_numeric($Vehicle_Year)) {
				if ((date("Y") - $Vehicle_Year) > 20) {
					$isOLD = "OLD2";
				} else if ((date("Y") - $Vehicle_Year) > 14) {
					$isOLD = "Y";
				}
			} else {
				$isOLD = "N";
			...
			if ($isAPU == "Y") {
				...
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
					$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=" . $warrantyID . " AND c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND v.Veh_ID = c.Veh_ID";
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						$cntrctRow = mysqli_fetch_assoc($cntrctResult);
						
						if (!empty($cntrctRow['APU_Flg']) && $cntrctRow['APU_Flg'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_APU_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_APU_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
			...
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						...
						if (!empty($cntrctRow['AEP_Flg']) && $cntrctRow['AEP_Flg'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_AEP_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_AEP_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
			...
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						...
						if (!empty($cntrctRow['Aerial_Flg']) && $cntrctRow['Aerial_Flg'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_AER_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_AER_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
			...
			if ($isWearable == "Y") {
				$query = "SELECT Dlr_Mrkp_Max_Amt FROM Addl_Std_Prcg WHERE Addl_Type_Cd = 'WEARABLES'";
				$result = $link->query($query);
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
					$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=" . $warrantyID . " AND c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND v.Veh_ID = c.Veh_ID";
			$cntrctResult = $link->query($query);
						...
						if (!empty($cntrctRow['wearables_flag']) && $cntrctRow['wearables_flag'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_WEARABLES_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_WEARABLES_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
							$Addl_Dlr_Mrkp_Actl_WEARABLES_Amt = $row["Dlr_Mrkp_Max_Amt"];
					}
					//$Addl_Dlr_Mrkp_Actl_WEARABLES_Amt = $row["Dlr_Mrkp_Max_Amt"];
				}
			}
			...
			if ($isEVBC == "Y") {
				$query = "SELECT Dlr_Mrkp_Max_Amt FROM Addl_Std_Prcg WHERE Addl_Type_Cd = 'EVBC'";
				$result = $link->query($query);
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
			...
			$cntrctResult = $link->query($query);
					...
					if (!empty($cntrctRow['EVBC_Flg']) && $cntrctRow['EVBC_Flg'] === 'Y') {
							$Addl_Dlr_Mrkp_Actl_EVBC_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_EVBC_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
					}
					}else{
							$Addl_Dlr_Mrkp_Actl_EVBC_Amt = $row["Dlr_Mrkp_Max_Amt"];
					}
					//$Addl_Dlr_Mrkp_Actl_EVBC_Amt = $row["Dlr_Mrkp_Max_Amt"];
				}
			}
			...
			if ($isEEC == "Y") {
				$query = "SELECT Dlr_Mrkp_Max_Amt FROM Addl_Std_Prcg WHERE Addl_Type_Cd = 'EEC'";
				$result = $link->query($query);
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
					$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=" . $warrantyID . " AND c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND v.Veh_ID = c.Veh_ID";
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						...
						if (!empty($cntrctRow['EEC_Flg']) && $cntrctRow['EEC_Flg'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_EEC_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_EEC_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
							$Addl_Dlr_Mrkp_Actl_EEC_Amt = $row["Dlr_Mrkp_Max_Amt"];
					}
					//$Addl_Dlr_Mrkp_Actl_EEC_Amt = $row["Dlr_Mrkp_Max_Amt"];
				}
			}
			...
			if ($isACP== "Y") {
				$query = "SELECT Dlr_Mrkp_Max_Amt FROM Addl_Std_Prcg WHERE Addl_Type_Cd = 'ACP'";
				$result = $link->query($query);
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
					$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=" . $warrantyID . " AND c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND v.Veh_ID = c.Veh_ID";
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						...
						if (!empty($cntrctRow['ACP_Flg']) && $cntrctRow['ACP_Flg'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_ACP_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_ACP_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
							$Addl_Dlr_Mrkp_Actl_ACP_Amt = $row["Dlr_Mrkp_Max_Amt"];
					}
					//$Addl_Dlr_Mrkp_Actl_ACP_Amt = $row["Dlr_Mrkp_Max_Amt"];
				}
			}
			...
			if ($isHUDS== "Y") {
				$query = "SELECT Dlr_Mrkp_Max_Amt FROM Addl_Std_Prcg WHERE Addl_Type_Cd = 'HUDS'";
				$result = $link->query($query);
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
					$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=" . $warrantyID . " AND c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND v.Veh_ID = c.Veh_ID";
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						...
						if (!empty($cntrctRow['HUDS_Flg']) && $cntrctRow['HUDS_Flg'] === 'Y') {
								$Addl_Dlr_Mrkp_Actl_HUDS_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_HUDS_Amt"];
						}
					}else{
						if($Addl_Dlr_Mrkp_Actl_HUDS_Amt===0 || $Addl_Dlr_Mrkp_Actl_HUDS_Amt==='0'){
							$Addl_Dlr_Mrkp_Actl_HUDS_Amt=0;
						}else{
							$Addl_Dlr_Mrkp_Actl_HUDS_Amt = $row["Dlr_Mrkp_Max_Amt"];
						}
							
					}
					//print_r($Addl_Dlr_Mrkp_Actl_HUDS_Amt);die;
					//$Addl_Dlr_Mrkp_Actl_HUDS_Amt = $row["Dlr_Mrkp_Max_Amt"];
				}
			}

			if ($isUCP== "Y") {
				$query = "SELECT Dlr_Mrkp_Max_Amt FROM Addl_Std_Prcg WHERE Addl_Type_Cd = 'UCP'";
				$result = $link->query($query);
				if ($result->num_rows > 0) {
					$row = mysqli_fetch_assoc($result);
					$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=" . $warrantyID . " AND c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND v.Veh_ID = c.Veh_ID";
			$cntrctResult = $link->query($query);
					if ($cntrctResult->num_rows > 0) {
						...
						if (!empty($cntrctRow['UCP_Flg']) && $cntrctRow['UCP_Flg'] === 'Y') {

								$Addl_Dlr_Mrkp_Actl_UCP_Amt =  $cntrctRow["Addl_Dlr_Mrkp_Actl_UCP_Amt"] ?? $row["Dlr_Mrkp_Max_Amt"];
						}
					}else{
							$Addl_Dlr_Mrkp_Actl_UCP_Amt = $row["Dlr_Mrkp_Max_Amt"];
					}
					//$Addl_Dlr_Mrkp_Actl_UCP_Amt = $row["Dlr_Mrkp_Max_Amt"];
				}
			}
			...
			*/
			...
			$addlStdPrcgResult = selectAddlStdPrcgSum($link, $isAEP, $isAPU, $isAER, $isOLD, $isEVBC, $isEEC, $isACP, $isHUDS, $isUCP, $isWearable);
			...
			$stmt = mysqli_prepare($link, "UPDATE Cntrct SET Addl_Sales_Agt_Cost_Amt=?, Addl_Sales_Agt_Commission_Amt=?,
										   ...
										   Addl_Dlr_Mrkp_Actl_AER_Amt=?,Addl_Dlr_Mrkp_Actl_WEARABLES_Amt=?,Addl_Dlr_Mrkp_Actl_EVBC_Amt=?,Addl_Dlr_Mrkp_Actl_EEC_Amt=?,Addl_Dlr_Mrkp_Actl_ACP_Amt=?,Addl_Dlr_Mrkp_Actl_HUDS_Amt=?,Addl_Dlr_Mrkp_Actl_UCP_Amt=? WHERE Cntrct_ID=?");
			...
			mysqli_stmt_bind_param($stmt, "iiiiiiiiiiiiiiii", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $Addl_Dlr_Mrkp_Actl_WEARABLES_Amt, $Addl_Dlr_Mrkp_Actl_EVBC_Amt, $Addl_Dlr_Mrkp_Actl_EEC_Amt, $Addl_Dlr_Mrkp_Actl_ACP_Amt, $Addl_Dlr_Mrkp_Actl_HUDS_Amt, $Addl_Dlr_Mrkp_Actl_UCP_Amt, $val10);
			...
			$stmt = mysqli_prepare($link, "UPDATE Cntrct SET Sales_Agt_Cost_Amt=?, Sales_Agt_Commission_Amt=?,
			                               Dlr_Cost_Amt=?, Dlr_Mrkp_Max_Amt=?,Additional_Commission_Amt=?, MSRP_Amt=? WHERE Cntrct_ID=?");
			...
			$val5 = $MSRP_Amt;
			$val6 = $contract_ID;
			$val7 = $Additional_Commission_Amt;

			mysqli_stmt_bind_param($stmt, "iiiiiii", $val1, $val2, $val3, $val4, $val7, $val5, $val6);
		}
	...
	if ($generatePdf) {
		...
		if ($Coverage_Term == '0.5') {
			...
			// $Coverage_Term = '3 YRS UNL'; // why this change?  naveender 10/18/2024
			$Coverage_Term = 3;
	...
	if ($smallGoodsPackage == "Y") {
		...
		if ($isQuote == "Y") {
			if (isset($_GET["warrantyID"]) != "" && $isEdited == false) {
				...
				if ($smallgoods > 0) {
					//Copy small goods
					copySmallGoods($link, $warrantyID);
				}
				header("location: small_goods_summary_worksheet.php?warrantyID=" . $warrantyID . "&isCopy=Y");
			} else {
							...
							<form name="dealerForm" id="warrantyForm" method="POST" action="">
								...
								<div class="form-row row">
									<div class="form-group col-md-6">
										...
										<!-- <?php
												if (isset($_GET["warrantyID"]) && $_GET["warrantyID"] != '' && !isset($_GET['isQuote']) && $isCreatedFromQuote < 1) {
												?>
											...
											?>
												<select class="form-control default-select" name="Acct_ID" id="sel1">\n
													...
													while ($row = mysqli_fetch_assoc($locationResult)) {
														$loopCounter++;
													?>
														<option value="<?php echo $row["Acct_ID"]; ?>" <?php if ($Mfr_Acct_ID == $row["Acct_ID"]) {
																											echo " selected ";
																										} ?>>
									...
									<div class="form-group col-md-6">
										...
										?>
											<select class="form-control default-select" name="dealerAgentID" id="sel1" data-size="7"
												...
												while ($row = mysqli_fetch_assoc($personResult)) {
													...
													$isSelected = "";
													if (
														$dealerAgentID == $row["Pers_ID"] ||
														($dealerAgentID == "" && $row["Cntct_Prsn_For_Acct_Flg"] == "Y")
													) {
														$isSelected = "selected";
													}

													// Build the display HTML
													$htmlContent = "<b>{$row['Pers_Frst_Nm']} {$row['Pers_Last_Nm']}</b><small>({$row['Email_URL_Desc']}) ({$row['Dlr_Loc_Nm']})</small><br>";

													if ($row["Cntct_Prsn_For_Acct_Flg"] == "Y") {
														$htmlContent .= "<small><em>Primary Contact</em></small>";
													}
												?>
													<option value="<?= $row['Pers_ID']; ?>" <?= $isSelected; ?>
														data-content='<?= $htmlContent; ?>'>
														<?= $row['Pers_Frst_Nm'] . " " . $row['Pers_Last_Nm']; ?>
													</option>
												<?php } ?>
											</select>
										<?php } ?>
									...
									<div class="form-group col-md-6">
										...
										<select class="form-control default-select" name="customerState" id="customerState" data-size="7"
											data-live-search="true">
									...
									<div class="form-group col-md-6">
										...
										<select class="form-control default-select" id="grossWeight" name="vehicleGrossWeight" onchange="weightChange(), fetchData()">
											...
											<option name="type4" value="type 4" <?php if ($Vehicle_Gross_Weight == "type 4") {
																					echo " selected ";
																					$tempType = "type 4";
																				} ?>>ARFF</option>
											<option name="type5" value="type 5" <?php if ($Vehicle_Gross_Weight == "type 5") {
																					echo " selected ";
																					$tempType = "type 5";
																				} ?>>POWERSPORTS</option>
										</select>
									...
									<div class="form-group col-md-6">
										...
										<input type="text" class="form-control" name="vehicleYear" id="vehicleYear" onchange="fetchData()" value="<?php echo $Vehicle_Year; ?>" autocomplete="off" />
									...
									<div class="form-group col-md-6">
										<label>Engine Serial # <?php if ($isQuote == 'N' && false) { ?> <span class="text-danger">*</span> <?php } ?></label>
									...
									<div style="display:none;">
										<div class="form-group col-md-6">
											...
											<div class="form-group mb-0">
												<!-- <label class="radio-inline mr-3"><input type="radio" value="Y" name="isAPU" <?php //if ($APU_Flg == "Y") {echo " checked='checked' ";} 
																																	?>> Yes</label> -->
												<label class="radio-inline mr-3"><input type="radio" value="N" checked name="isAPU" <?php //if ($APU_Flg == "N") {echo " checked='checked' ";} 
																																	?>> No</label>
											</div>
										...
										<div class="form-group col-md-6">
											...
											<input type="hidden" class="form-control" name="apuMake" id="apuMake" value="<?php echo $APU_Engine_Make; ?>" />
										...
										<div class="form-group col-md-6">
											...
											<input type="hidden" class="form-control" name="apuModel" id="apuModel" value="<?php echo $APU_Engine_Model; ?>" />
										...
										<div class="form-group col-md-6">
											...
											<input type="hidden" class="form-control" name="apuYear" id="apuYear" value="<?php echo $APU_Engine_Year; ?>" />
										...
										<div class="form-group col-md-6">
											...
											<input type="hidden" class="form-control" name="apuSerialNumber" id="apuSerialNumber" value="<?php echo $APU_Engine_Serial; ?>" />
										...
										<div class="form-group col-md-6">
											...
											<input type="hidden" class="form-control" name="apuHours" id="apuHours" value="<?php echo $APU_Hours; ?>" />
									...
									<div class="form-group col-md-6" style="display:none;">
										...
										<div class="form-group mb-0" >
											<label class="radio-inline mr-3 isVehicleNew"><input type="radio" value="Y" name="isVehicleNew" <?php if ($Vehicle_New_Flag == "Y") {
																																				echo " checked='checked' ";
																																			} ?>> New</label>
											<label class="radio-inline mr-3 isVehicleNew"><input type="radio" value="N" name="isVehicleNew" <?php if ($Vehicle_New_Flag == "N") {
																																				echo " checked='checked' ";
																																			} ?>> Used</label>
										</div>
									...
									<?php
									//var_dump($role_ID, $isQuote, $warrantyID);
									if($role_ID == 1 && empty($warrantyID) || ($isQuote == "Y" && empty($warrantyID))){ ?>
									<div class="form-group col-md-12">
										<h4>Select Wholesale or Standard</h4>
									</div>
									<div class="form-group col-md-6">
										<label>Wholesale or Standard<span class="text-danger">*</span></label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="wholesale" name="wrntyStatDesc" onchange="fetchData()" <?php if (isset($Wholesale_Flg) && $Wholesale_Flg === "Y") {
																																						echo " checked='checked' ";
																																					} ?>> Wholesale</label>
											<label class="radio-inline mr-3"><input type="radio" value="standard" name="wrntyStatDesc" onchange="fetchData()" <?php if (isset($Wholesale_Flg) && $Wholesale_Flg === "N") {
																																						echo " checked='checked' ";
																																					} ?>> Standard</label>
										</div>
										<!-- <span style="color: red;display: none;" id="vehicleTierTypeE">Please Select Wearable Type.! <span class="text-danger">*</span></span> -->
									</div>
									<?php } ?>
									<div class="form-group col-md-12">
									...
									<div class="form-group col-md-6">
										<label>Wearables Package <span class="text-danger">*</span></label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="wearables" onchange="fetchData()" <?php if (isset($wearables_flag) && $wearables_flag == "Y") {
																																						echo " checked='checked' ";
																																					} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="wearables" onchange="fetchData()" <?php if (isset($wearables_flag) && $wearables_flag == "N") {
																																						echo " checked='checked' ";
																																					} ?>> No</label>
										</div>
										<!-- <span style="color: red;display: none;" id="vehicleTierTypeE">Please Select Wearable Type.! <span class="text-danger">*</span></span> -->
									...
									<div class="form-group col-md-6">
										...
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="S" name="vehicleTierType" onchange="fetchData()" <?php if ($Tier_Type == "S") {
																																								echo " checked='checked' ";
																																							} ?>> Squad</label>
											<label class="radio-inline mr-3"><input type="radio" value="B" name="vehicleTierType" onchange="fetchData()" <?php if ($Tier_Type == "B") {
																																								echo " checked='checked' ";
																																							} ?>> Battalion</label>
										</div>
									...
									<div class="form-group col-md-6">
										<label>Fire Apparatus Pump & Equipment Package <span class="text-danger type2Field"></span></label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="boltOnPackage" onchange="fetchData()" <?php if ($Apparatus_Equipment_Package == "Y") {
																																							echo "checked='checked' ";
																																						} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="boltOnPackage" onchange="fetchData()" <?php if ($Apparatus_Equipment_Package == "N") {
																																							echo "checked='checked' ";
																																						} ?>> No</label>
										</div>
									...
									<div class="form-group col-md-6">
										...
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="aerialPackage" onchange="fetchData()" <?php if ($Aerial_Package == "Y") {
																																							echo " checked='checked' ";
																																						} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="aerialPackage" onchange="fetchData()" <?php if ($Aerial_Package == "N") {
																																							echo " checked='checked' ";
																																						} ?>> No</label>
										</div>
									...
									<div class="form-group col-md-6">
										<label>Small Goods</label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="smallGoodsPackage" onchange="fetchData()" <?php if ($smallGoodsPackage == "Y") {
																																								echo " checked='checked' ";
																																							} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="smallGoodsPackage" onchange="fetchData()" <?php if ($smallGoodsPackage == "N") {
																																								echo " checked='checked' ";
																																							} ?>> No</label>
										</div>
									...
									<div class="form-group col-md-6">
										<label>Electric Vehicle Battery Package	<span class="text-danger type3Field">*</span></label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="isEVBC" onchange="fetchData()" <?php if ($EVBC_Flg == "Y") {
																																					echo " checked='checked' ";
																																				} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="isEVBC" onchange="fetchData()" <?php if ($EVBC_Flg == "N") {
																																					echo " checked='checked' ";
																																				} ?>> No</label>
										</div>
										<span style="color: red;display: none;" id="isEVBC">Please Select Electric Vehicle Battery Coverage..!</span>
									</div>
									<div class="form-group col-md-6">
										<label>Enhanced Engine Coverage <span class="text-danger type3Field">*</span></label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="isEEC" onchange="fetchData()" <?php if ($EEC_Flg == "Y") {
																																					echo " checked='checked' ";
																																				} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="isEEC" onchange="fetchData()" <?php if ($EEC_Flg == "N") {
																																					echo " checked='checked' ";
																																				} ?>> No</label>
										</div>
										<span style="color: red;display: none;" id="isEEC">Please Select Enhanced Engine Coverage..!</span>
									</div>
									<!-- New Addons -->

									<div class="form-group col-md-6">
										<label>Ambulance Conversion Package</label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="isACP" onchange="fetchData()" <?php if ($ACP_Flg == "Y") {
																																					echo " checked='checked' ";
																																				} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="isACP" onchange="fetchData()" <?php if ($ACP_Flg == "N") {
																																					echo " checked='checked' ";
																																				} ?>> No</label>
										</div>
										<span style="color: red;display: none;" id="isACP">Please Select Ambulance Conversion Package..!</span>
									</div>
									<?php
									if (isset($_SESSION['admin_loggedin']) && $_SESSION['admin_loggedin'] == true) {
									?>
									<div class="form-group col-md-6">
										<label>High Use Dept Surcharge</label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="isHUDS" onchange="fetchData()" <?php if ($HUDS_Flg == "Y") {
																																					echo " checked='checked' ";
																																				} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="isHUDS" onchange="fetchData()" <?php if ($HUDS_Flg == "N") {
																																					echo " checked='checked' ";
																																				} ?>> No</label>
										</div>
										<span style="color: red;display: none;" id="isHUDS">Please Select High Use Dept Surcharges..!</span>
									</div>
									<?php }else{ ?>
										<input type="hidden" name="isHUDS" value="<?php echo $HUDS_Flg; ?>">
										<?php } ?>
									<!-- End New Addons -->
									<!-- Start UPFITTER CONVERSION PACKAGE -->
									<div class="form-group col-md-6">
										<label>Upfitter Conversion Package</label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="isUCP" onchange="fetchData()" <?php if ($UCP_Flg == "Y") {
																																					echo " checked='checked' ";
																																				} ?>> Yes</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="isUCP" onchange="fetchData()" <?php if ($UCP_Flg == "N") {
																																					echo " checked='checked' ";
																																				} ?>> No</label>
										</div>
										<span style="color: red;display: none;" id="isUCP">Please Select Upfitter Conversion Package..!</span>
									</div>
									<!-- End UPFITTER CONVERSION PACKAGE -->

									<div class="form-group col-md-6 wrap-section">
										<label>Wrap Program <span class="text-danger">*</span></label>
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="wrap_program" onchange="fetchData()" <?php if ($wrap_program == "Y") {
																																							echo " checked='checked' ";
																																						} ?> disabled> Yes</label>
											<?php if ($warrantyID != '') { ?>
												<label class="radio-inline mr-3"><input type="radio" value="N" name="wrap_program" onchange="fetchData()" <?php if ($wrap_program == "N") {
																																								echo " checked='checked' ";
																																							} ?> disabled> No</label>
											<?php } else { ?>
												<label class="radio-inline mr-3"><input type="radio" value="N" name="wrap_program" onchange="fetchData()" checked> No</label>
											<?php } ?>

										</div>
										<div class="form-group mb-0 wrap-years d-none">
											<label class="radio-inline mr-3 wrapProgram"><input type="radio" value="1" name="wrapProgram2" onchange="fetchData()" <?php if ($wrap_Program_Term == "1") {
																																										echo " checked='checked' ";
																																									} ?>> 1 year</label>
											<label class="radio-inline mr-3 wrapProgram"><input type="radio" value="2" name="wrapProgram2" onchange="fetchData()" <?php if ($wrap_Program_Term == "2") {
																																										echo " checked='checked' ";
																																									} ?>> 2 year</label>
											<label class="radio-inline mr-3 wrapProgram"><input type="radio" value="3" name="wrapProgram2" onchange="fetchData()" <?php if ($wrap_Program_Term == "3") {
																																										echo " checked='checked' ";
																																									} ?>> 3 year</label>
											<label class="radio-inline mr-3 wrapProgram"><input type="radio" value="4" name="wrapProgram2" onchange="fetchData()" <?php if ($wrap_Program_Term == "4") {
																																										echo " checked='checked' ";
																																									} ?>> 4 year</label>
											<label class="radio-inline mr-3 wrapProgram"><input type="radio" value="5" name="wrapProgram2" onchange="fetchData()" <?php if ($wrap_Program_Term == "5") {
																																										echo " checked='checked' ";
																																									} ?>> 5 year</label>
										</div>
									...
									<div class="form-group col-md-6 coverage-term-section" id="coverage-section">
										...
										<div class="form-group mb-0">
											<select class="form-control default-select" name="coverageTerm" id="coverageTerm" onchange="fetchData()">
												<option value="0.5" <?php if ($Coverage_Term == "0.5") {
																		echo " selected ";
																	} ?>>6 Months</option>
												<option value="1" <?php if ($Coverage_Term == "1") {
																		echo " selected ";
																	} ?>>1 year</option>
												<option value="2" <?php if ($Coverage_Term == "2") {
																		echo " selected ";
																	} ?>>2 years</option>
												<option value="3" <?php if ($Coverage_Term == "3") {
																		echo " selected ";
																	} ?>>3 years</option>
												<option value="4" <?php if ($Coverage_Term == "4") {
																		echo " selected ";
																	} ?>>4 years</option>
												<option value="5" <?php if ($Coverage_Term == "5") {
																		echo " selected ";
																	} ?>>5 years</option>
												<option value="6" <?php if ($Coverage_Term == "6") {
																		echo " selected ";
																	} ?>>6 years</option>
												<option value="7" <?php if ($Coverage_Term == "7") {
																		echo " selected ";
																	} ?>>7 years</option>
												<option value="8" <?php if ($Coverage_Term == "8") {
																		echo " selected ";
																	} ?>>8 years</option>
												<option value="9" <?php if ($Coverage_Term == "9") {
																		echo " selected ";
																	} ?>>9 years</option>
												<option value="10" <?php if ($Coverage_Term == "10") {
																		echo " selected ";
																	} ?>>10 years</option>
											</select>
										</div>
									...
									?>
										<div class="form-group col-md-6">
											...
											<select class="form-control default-select" id="customerSalesChannel" name="customerSalesChannel">
												<option value="Outside Sales" <?php if ($customerSalesChannel == "Outside Sales") {
																					echo " selected ";
																				} ?>>Outside Sales</option>
												<option value="Inside Sales" <?php if ($customerSalesChannel == "Inside Sales") {
																					echo " selected ";
																				} ?>>Inside Sales</option>
											</select>
											<span style="color: red;display: none;" id="customerSalesChannelE">Please Enter Contract Sales Channel..!</span>
									...
									<div class="form-group col-md-6">
										...
										<div class="form-group mb-0">
											<label class="radio-inline mr-3"><input type="radio" value="Y" name="Srvc_Veh_Flg" <?php
																																if ($Srvc_Veh_Flg == "Y") {
																																	echo " checked='checked' ";
																																} ?>>
												Yes
											</label>
											<label class="radio-inline mr-3"><input type="radio" value="N" name="Srvc_Veh_Flg" <?php
									...
									?>
										...
										<div class="form-group col-md-6">
											...
											<input type="date" class="form-control" name="supplyPacketShippedDate" id="supplyPacketShippedDate" value="<?php if ($Supply_Packet_Shipped_Date != "") {
																																							$explodedDate = explode(" ", $Supply_Packet_Shipped_Date);
																																							echo $explodedDate[0];
																																						} ?>">
									...
									<div class="form-group col-md-6">
										...
										<select class="form-control default-select" name="lienHolderState" id="lienHolderState" data-size="7"
											data-live-search="true">
...
<script>
	(function($) {
    "use strict"
		var vehicleYear = "<?php echo (int)$Vehicle_Year; ?>";
 
		$("#vehicleYear").yearpicker({
			startYear: 1900,
			endYear: new Date().getFullYear()+20,
			autoHide: true,
			year: <?php echo (int)$Vehicle_Year; ?>
		});
	...
	function warrantyRules() {
		...
		var NewVehicleDate = vehicleDate.getFullYear() + 1;
		...
		if (new Date() < new Date(2023, 3, 1)) {
			var OldVehicleDate = vehicleDate.getFullYear() + 32;
		} else {
			var OldVehicleDate = vehicleDate.getFullYear() + 31;
		}

		if(NewVehicleDate < currentYear){
			$("input[name=isVehicleNew][value='N']").prop('checked', true);
		} else {
			$("input[name=isVehicleNew][value='Y']").prop('checked', true);
		}


		if ($("#odometerReading").val() > 500 && $("input:radio[name='milesOrKM']:checked").val() == "miles" && NewVehicleDate < currentYear) {
			Swal.fire({
				position: 'top-center',
				title: 'ERROR',
	...
	function fetchData() {
		var currentYear = new Date().getFullYear();
		var vehicleDate = new Date($("#vehicleYear").val()).getFullYear();
		var vehicleAge = currentYear - vehicleDate;
		var oldTag = '';

		if (vehicleAge > 14 && vehicleAge <= 20) {
			oldTag = 'OLD';
		...
		var categoryId = 55;
		var terms = $('#coverageTerm').val();
		var vehicleType = $('#grossWeight').val();
		var coverageType = $('input[name="vehicleTierType"]:checked').val();
		var wearables = $('input[name="wearables"]:checked').val();
		var aep = $('input[name="boltOnPackage"]:checked').val();
		var aerialPackage = $('input[name="aerialPackage"]:checked').val();
		var evbcCoverage = $('input[name="isEVBC"]:checked').val();
		var eecCoverage = $('input[name="isEEC"]:checked').val();
		var acpCoverage = $('input[name="isACP"]:checked').val();
		var hudsCoverage =$('input[name="isHUDS"]:checked').val() ?? $('input[name="isHUDS"][type="hidden"]').val();
		var ucpCoverage = $('input[name="isUCP"]:checked').val();
		var wrapProgramm = $('input[name="wrap_program"]:checked').val();
		var wrapProgrammTerms = $('input[name="wrapProgram2"]:checked').val();
		var smallGoodsPackage = $('input[name="smallGoodsPackage"]:checked').val();
		var wrntyStatDesc = $('input[name="wrntyStatDesc"]:checked').val();
		if (wrapProgramm == 'Y') {
			terms = wrapProgrammTerms;
		}
		$('#price-table').html('');
		...
		if (vehicleType == 'type 1') {
			vehicleType = 1;
		} else if (vehicleType == 'type 2') {
			vehicleType = 2;
		} else if (vehicleType == 'type 3') {
			vehicleType = 3;
		} else if (vehicleType == 'type 4') {
			vehicleType = 4;
		} else if (vehicleType == 'type 5') {
			vehicleType = 5;
		}


		if (terms && vehicleType && coverageType) {
			$.ajax({
				url: 'pricing_table.php',
				type: 'GET',
				data: {
					warrantyID: "<?php echo isset($_GET['warrantyID']) ? $_GET['warrantyID'] : ''; ?>",
					terms: terms,
					...
					ajax: true,
					aep: aep,
					aerialPackage: aerialPackage,
					evbcCoverage: evbcCoverage,
					eecCoverage: eecCoverage,
					acpCoverage: acpCoverage,
					hudsCoverage: hudsCoverage,
					ucpCoverage: ucpCoverage,
					wrapProgramm: wrapProgramm,
					smallGoodsPackage: smallGoodsPackage,
					isQuote: "<?php echo $isQuote; ?>",
					_t: new Date().getTime(),
					oldTag: oldTag,
					wrntyStatDesc: wrntyStatDesc ? wrntyStatDesc : null
				...
				success: function(response) {
					...
					$('#price-table').html(response); // Inject rows into table body
				},
				error: function() {
			...
			$('#price-table').html(''); // Clear table if no selection
		}
	}
	...
	$(document).ready(function() {
		...
		if (isWrap == 'Y') {
			...
			$('input[name=wrapProgram2]').attr("disabled", false);

		} else {
			//cp$('input[name=wrapProgram]').attr("disabled",true);
			$('input[name=coverageTerm]').attr("disabled", false);
			$("#wrapProgramE").css("display", "none"); // cparry 6/15/2023
		}

		$("input:radio[name='wrapProgram2']").on("change", function() {
			var isWrapTypeYear = parseInt($("input:radio[name='wrapProgram2']:checked").val(), 10);
			if (isWrapTypeYear == 1 || isWrapTypeYear == 2 || isWrapTypeYear == 3 || isWrapTypeYear == 4 || isWrapTypeYear == 5) {
		...
		if (document.getElementById("grossWeight").value == 'type 1') {
			...
			$('input[name=wrapProgram2]').attr("disabled", true);
		...
		} else if (document.getElementById("grossWeight").value == 'type 4') {
		...
		} else if (document.getElementById("grossWeight").value == 'type 5') {
		...
		$("#odometerReading, #vehicleYear, #grossWeight, .milesOrKM").change(function() {

			var currentYear = new Date().getFullYear();
			var vehicleDate = new Date($("#vehicleYear").val());
			var NewVehicleDate = vehicleDate.getFullYear() + 1;
			var NewVehicleDateForWrap = vehicleDate.getFullYear() + 4;
			...
			if(NewVehicleDateForWrap < currentYear){
				//Wrap Program
				$("input:radio[name='wrap_program']").attr('disabled', true);
				$('input[name=wrapProgram2]').attr("disabled", true);
				$("input[name=wrap_program][value='N']").prop('checked', true);
				//var OldVehicleDate = vehicleDate.getFullYear() + 21;
			} else {
				//Wrap Program
				$("input:radio[name='wrap_program']").attr('disabled', false);
			...
			} else if ($("#odometerReading").val() > 804 && $("input:radio[name='milesOrKM']:checked").val() == "km" && NewVehicleDate < currentYear) {
				Swal.fire({
					position: 'top-center',
					title: 'ERROR',
					html: "<p class='text-danger'>Vehicle older than 2 years. This is a USED vehicle, not a new vehicle. Please change the checkbox to USED VEHICLE.</p>",
			...
			} else if (NewVehicleDate < currentYear) {
				Swal.fire({
					position: 'top-center',
					title: 'ERROR',
			...
			} else if (OldVehicleDate < currentYear) {
			...
			} else if ($("input:radio[name='isVehicleNew']:checked").val() == 'N' && $("#grossWeight").val() == 'type 4' && $("#odometerReading").val() > 250000 && $("input:radio[name='milesOrKM']:checked").val() == "miles") {
				Swal.fire({
					position: 'top-center',
					title: 'ERROR',
					html: "<p class='text-danger'>Vehicle has more than 250,000 miles, and cannot be warrantied.</p>",
			...
			} else if ($("input:radio[name='isVehicleNew']:checked").val() == 'N' && $("#grossWeight").val() == 'type 4' && $("#odometerReading").val() > 402336 && $("input:radio[name='milesOrKM']:checked").val() == "km") {
				Swal.fire({
					position: 'top-center',
					title: 'ERROR',
					html: "<p class='text-danger'>Vehicle has more than 402336 kilometers, and cannot be warrantied.</p>",
		...
		$('input').on('keypress', function(e) {
			if (e.which === 13) {
		        ...
		        */
				...
				if ($("#apuMake").val() == "" && $("input[type=radio][name=isAPU]:checked").val() == "Y" && isQuote == "N") {
				...
				if ($("#apuModel").val() == "" && $("input[type=radio][name=isAPU]:checked").val() == "Y" && isQuote == "N") {
				...
				if ($("#apuYear").val() == "" && $("input[type=radio][name=isAPU]:checked").val() == "Y" && isQuote == "N") {
				...
				if ($("#apuSerialNumber").val() == "" && $("input[type=radio][name=isAPU]:checked").val() == "Y" && isQuote == "N") {
				...
				if ($("#apuHours").val() == "" && $("input[type=radio][name=isAPU]:checked").val() == "Y" && isQuote == "N") {
				...
				if ($("input:radio[name='boltOnPackage']").is(":checked") && ($("#vehicleType").val() == 'type 5' || $("#vehicleType").val() == 'type 4' || $("#vehicleType").val() == 'type 3' || $("#vehicleType").val() == 'type 2') || $("#vehicleType").val() == 'type 1') {
				...
				if ($("input:radio[name='aerialPackage']").is(":checked") && ($("#vehicleType").val() == 'type 5' || $("#vehicleType").val() == 'type 4' || $("#vehicleType").val() == 'type 3' || $("#vehicleType").val() == 'type 2') || $("#vehicleType").val() == 'type 1') {
				...
				if (($("input:radio[name='wrapProgram2']").is(":checked")) || ($('input[name=wrapProgram2]').attr("disabled", true))) {
					$("#wrapProgramE").css("display", "none");
					flag51 = 1;
				} else {
		...
		$("#warrantyFormSubmit").click(function() {
			...
			if ($("input:radio[name='boltOnPackage']").is(":checked") && ($("#vehicleType").val() == 'type 3' || $("#vehicleType").val() == 'type 2' || $("#vehicleType").val() == 'type 4' || $("#vehicleType").val() == 'type 5') || $("#vehicleType").val() == 'type 1') {
				$("#boltOnPackageE").css("display", "none");
				flag42 = 1;
			} else {
				Error_message += "<li> AEP is required</li>";
			...
			if ($("input:radio[name='aerialPackage']").is(":checked") && ($("#vehicleType").val() == 'type 3' || $("#vehicleType").val() == 'type 2' || $("#vehicleType").val() == 'type 4' || $("#vehicleType").val() == 'type 5') || $("#vehicleType").val() == 'type 1') {
				$("#aerialPackageE").css("display", "none");
				flag43 = 1;
			} else {
				Error_message += "<li> Aerial package is required</li>";
			...
			if (($("input[type=radio][name=wrap_program]:checked").val() == "Y")) {
				if (($("#grossWeight").val() == 'type 3' || $("#grossWeight").val() == 'type 4') && $("input:radio[name='vehicleTierType']:checked").val() == 'B') {
					flag51 = 1;

				} else {
					Error_message += "<li>You have to select Tier Type Battalion and Type 3, Type 4 for Wrap program, and choose a correct term (1, 2, 3, 4, or 5 years) as well.</li>";
					flag51 = 0;
				}
			} else {
				$('input[name=wrapProgram2]').attr("disabled", true)
			...
			if ($("input[type=radio][name=wrap_program]:checked").val() == "Y") {
				if (!$("input[name='wrapProgram2']:checked").val()) {
					Error_message += "<li> Wrap Program is required</li>";
					$("#wrapProgramE").css("display", "block");
					flag51 = 0;
				}
			...
			if ($("input[type=radio][name=wrap_program]:checked").val() == "Y") {
				// Error_message += "<li> Wrap Program field is required</li>";
				$("#coverageTermE").css("display", "none");
				$("#wrapProgramE").css("display", "block");
				flag45 = 1;
			}
			...
			if (flag1 == 1 &&
				...
				if (warrantyId != "") {
					...
					if (isQuote == "Y") {
						...
						swalWithBootstrapButtons.fire({
							title: 'Do you want to update this Warranty or create a new Warranty?',
							icon: 'warning',
							showCancelButton: true,
							confirmButtonText: 'Create a New Warranty',
							cancelButtonText: 'Update this Warranty',
							reverseButtons: true
						}).then((result) => {
							if (result.isConfirmed) {
								$("#warrantyFormSubmit").prop("disabled", true);
					...
					if ($("input[type=radio][name=wrap_program]:checked").val() == "Y") {
						...
						swalWithBootstrapButtons.fire({
							title: 'The Wrap Program does not cover the engine, transmission, or differential. Please click OK at this point to proceed.',
							icon: 'warning',
							showCancelButton: true,
...
<script>
	function weightChange() {
		...
		var isQuote = $("#isQuote").text();
		if (document.getElementById("grossWeight").value == 'type 4' || document.getElementById("grossWeight").value == 'type 3' || document.getElementById("grossWeight").value == 'type 2') {
			$(".type2Field").text('*');
		} else {
			$(".type2Field").text('');
		}
		...
		if (document.getElementById("grossWeight").value == 'type 4' && isQuote == 'N') {
			$(".type3Field").text('*');
		} else {
			$(".type3Field").text('');
		}
		...
		if (document.getElementById("grossWeight").value == 'type 1') {
			...
			$('input[name=wrapProgram2]').attr("disabled", true);
			$('.wrap-years').addClass("d-none");
			// $('.wrap-section').addClass('d-none');
			$('input[name=isUCP]').attr("disabled", false);
			...
			*/
			...
			$('input[name=isUCP]').attr("disabled", true);
			$("input[name=isUCP][value='N']").prop('checked', true);
		}
		else if(document.getElementById("grossWeight").value == 'type 4') {
			$('input[name=isUCP]').attr("disabled", true);
			$("input[name=isUCP][value='N']").prop('checked', true);
			...
			if ($("input:radio[name='isVehicleNew']:checked").val() == 'Y') {
				$('input[name=wrap_program]').attr("disabled", false);
			}

			$('input[type=radio][name=isVehicleNew]').change(function() {
				...
				if (this.value == 'N') {
					...
					// removed  && $("input:radio[name='vehicleTierType']:checked").val() == 'B'
					if ($("#grossWeight").val() == 'type 3') {
						//$('input[name=coverageTerm]').attr("disabled",true);
						//$("input[name=wrap_program][value='Y']").prop('checked', true);
						$('input[name=wrap_program]').attr("disabled", false);
						//$(".wrap-years").removeClass("d-none");
						//$('input[name=wrapProgram2]').attr("disabled",false);
						$("#wrapProgramE").css("display", "none"); // cparry 6/15/2023.
					} else if ($("#grossWeight").val() == 'type 3') {
						Error_message += "<li>You have to select Tier Type Battalion and Type 3 for Wrap program, and choose a correct term (2, 3, or 5 years) as well.</li>";
						Swal.fire({
							position: 'top-center',
							title: 'Please enter required fields',
							html: "<ul style='padding: 0 0 0 38px;text-align: left;color: red;'>" + Error_message + "</ul>",
					...
					if ($("#grossWeight").val() == 'type 4') {
						...
						//$('input[name=wrapProgram2]').attr("disabled",false);
						$("#wrapProgramE").css("display", "none"); // cparry 6/15/2023.
					} else if ($("#grossWeight").val() == 'type 4') {
						Error_message += "<li>You have to select Tier Type Battalion and Type 4 for Wrap program, and choose a correct term (2, 3, or 5 years) as well.</li>";
						Swal.fire({
							position: 'top-center',
							title: 'Please enter required fields',
							html: "<ul style='padding: 0 0 0 38px;text-align: left;color: red;'>" + Error_message + "</ul>",
	...
	$("input:radio[name='wrap_program']").change(function() {
		if (this.value == 'N') {
			...
			if (($("#grossWeight").val() == 'type 3' || $("#grossWeight").val() == 'type 4') && $("input:radio[name='vehicleTierType']:checked").val() == 'B') {
				...
				var Error_message = "<li>You have to select Tier Type Battalion and Type 3, Type 4 for Wrap program, and choose a correct term (1, 2, 3, 4 or 5 years) as well.</li>";
				Swal.fire({
					position: 'top-center',
			...
			if ($("input[name='wrapProgram2']:checked").length > 0) {
				$("#wrapProgramE").css("display", "none");
			} else {
				$("#wrapProgramE").css("display", "block");
			}
