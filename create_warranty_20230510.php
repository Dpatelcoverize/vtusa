...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (($warrantyID != "") && $isEdited ) {
		...
		$stmt = mysqli_prepare($link, "INSERT INTO Cntrct (Cntrct_Nbr,Cntrct_Sales_Chnl,Sply_Pkt_Shipd_Dte,
									   Cntrct_Dim_ID,Veh_ID,Mfr_Acct_ID,Pers_Who_Signd_Cntrct_ID,Dlr_Agt_Prsn_ID,Created_Date)
									   VALUES (?,?,?,?,?,?,?,?,NOW())");
		...
		mysqli_stmt_bind_param($stmt, "sssiiiii", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8);
   ...
   // Generate PDF	// Start PDF Code here....
	...
	if ($phoneResult) {
		...
		if ($generatePdf) {
			...
			$agreeDate = '<u>' . $agreementDate . '</u>';
			$assignDate = date('d-m-Y', strtotime($agreementDate));

/*
echo "agreementDate=".$agreementDate;
echo "<br/>";
echo "agreeDate=".$agreeDate;
die();
*/
			//Customer State
			...
			if($isQuote == "Y")
			{
				...
				if($Tier_Type == 'S' || $Tier_Type == 'SQUAD' )
				{
					$pdf_Tier_Type = 'Squad';
				}
				elseif($Tier_Type == 'B' || $Tier_Type == 'BATTALION')
				{
				...
				elseif($type == 'TYPE 2'){
					...
					if(($Apparatus_Equipment_Package == "YES") || ($isAEP=="Y")){
						$pdf_Apparatus_Equipment_Package = "Yes";
						$componentCoverageHtml .= '
								<tr>
								<td style="padding:7px 10px; font-size:13px;" class="border-bottom-none">
				...
				elseif($type == 'TYPE 3'){
					...
					if(($Apparatus_Equipment_Package == "YES") || ($isAEP=="Y")){
						...
						$componentCoverageHtml .= '
							<tr>
								<td style="padding:7px 10px; font-size:13px;" class="border-bottom-none">
									<label><input type="checkbox" name=""> APPARATUS EQUIPMENT PACKAGE: ' .$pdf_Apparatus_Equipment_Package . '</label>
								</td>
							</tr>
						';
					}

					if($Aerial_Package == "YES"){
						$pdf_Aerial_Package = "Yes";
						$componentCoverageHtml .= '
							<tr>
								<td style="padding:7px 10px; font-size:13px;" class="border-bottom-none">
				...
				*/
							...
							<tr>
								<td>
									<table class="inner-full-width"  cellpadding="2" cellspacing="2" style="margin:0;">
										...
										<tr>
											<th colspan="6" style="padding:7px 10px; font-size:13px;"><b>VEHICLE DESCRIPTION:</b>  '.$Vehicle_Description.'</th>
										</tr>
										<tr>
				...
				$query = "SELECT cd.Cntrct_Dim_ID FROM Cntrct c, Cntrct_Dim cd WHERE c.Mfr_Acct_ID=" . $Mfr_Acct_ID . " AND c.Cntrct_Dim_ID = cd.Cntrct_Dim_ID;";
				$result = $link->query($query);
				$row = $result->fetch_assoc();
			...
			else
			{
				...
				$data = [
					...
					$pdf = new GeneratePDF;
					$pdf->generate($data , $pdfFileName , $Vehicle_Type , $isQuote);
						...
						$primary_Contact_Person_id = $row['Pers_ID'];
								...
								// Get the contract info
						$query = "SELECT cd.Cntrct_Dim_ID, cd.Assign_Rtlr_Nbr FROM Cntrct c, Cntrct_Dim cd WHERE c.Mfr_Acct_ID=" . $Mfr_Acct_ID . " AND c.Cntrct_Dim_ID = cd.Cntrct_Dim_ID;";
						$result = $link->query($query);
						$row = $result->fetch_assoc();
		...
		if ($smallGoodsPackage == "Y") {
			...
			// Redirect to next form
		 	...
		 	if ($isQuote == "Y") {
				...
				{
					header("location: small_goods_summary_worksheet.php?copyQuote=".$_GET["warrantyID"]);
				} else{
				...
				// if(!empty(trim($_POST["warrantyID"])))
				// {
				// 	header("location: small_goods_summary_worksheet.php?copyQuote=true");
				// } else {
	...
	$(document).ready(function(){
	 ...
	 //Validation for warranty/quote form submission
	    ...
	    if(document.getElementById("grossWeight").value == 'type 1')
		{
			...
			/*
			// Re-enable the APU field for all types, 5/3/2023 cparry.
			//APU
			$("input:radio[name='isAPU']").each(function(i) {
...
<?php
/***
// Removed the following javascript that had a mileage check:
				else if( $("#odometerReading").val() > 500 && $("input:radio[name='milesOrKM']:checked").val() == "miles" && $("input:radio[name='isVehicleNew']:checked").val() == 'Y')
				{
					...
					Swal.fire({
						position: 'top-center',
						title: 'ERROR',
						html: "<p class='text-danger'>Vehicle mileage greater than 500 miles.  This is a USED vehicle, not a new vehicle.</p>",
						showConfirmButton: true,
					});
...
/******
// Removed the following javascript to check mileage:
else if( $("#odometerReading").val() > 500 && $("input:radio[name='milesOrKM']:checked").val() == "miles" && $("input:radio[name='isVehicleNew']:checked").val() == 'Y')
				{
					...
					const swalWithBootstrapButtons = Swal.mixin({
						...
						swalWithBootstrapButtons.fire({
						 ...
						 reverseButtons: true
						  ...
						  // $("#warrantyForm").append("<input type='hidden' name='Edited' value='false'/>");
						  $("#warrantyForm").submit();
							setTimeout(function() {
...
/*******
// Removed the following javascript to check odometer for new:
 else if( $("#odometerReading").val() > 500 && $("input:radio[name='milesOrKM']:checked").val() == "miles" && $("input:radio[name='isVehicleNew']:checked").val() == 'Y')
			{
				Error_message += "<li>Vehicle mileage greater than 500 miles.  This is a USED vehicle, not a new vehicle.</li>";
				flag48 = 0;
