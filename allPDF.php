...
if ($dealerID != "" && $userType == "dealer") {
    ...
    $fileTypeID = $_GET['fileTypeID'];
    $type = "";
     /* get Pdf link */
    $queryString = "SELECT Path_to_File FROM File_Assets WHERE Acct_ID=" . $dealerID ." AND File_Asset_Type_ID = ".$fileTypeID;
    $typeQuery = "SELECT File_Asset_Type FROM File_Asset_Types WHERE File_Asset_Type_ID =". $fileTypeID;

    $allPDF = $link->query($queryString);
    $pdf_type = $link->query($typeQuery);

    while ($row = $pdf_type->fetch_assoc()) {

        $type =  $row['File_Asset_Type'];
    }
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header text-center">
            <h4 class="card-title"><? echo $type ?> PDF Files</h4>
          </div>
          <div class="card-body">
