...
class TCPDF {
	 ...
	 */
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false,$footer_html=new stdClass()) {
	...
	public function Footer() {
		...
		if (empty($this->pagegroups)) {
			...
			$pagenumtxt = $w_page.'Page '.$this->getAliasNumPage();
		} else {
		...
		if ($this->getRTL()) {
			...
			$this->setFont($cambriabF, '', 10, '', true);
			$this->SetTextColor(139,139,139);

			$celll = property_exists($this->footer_html, 'cellL') ? $this->footer_html->cellL : '';
			$cellM = property_exists($this->footer_html, 'cellM') ? $this->footer_html->cellM : '';

			$footerHTML = '<table cellpadding="4" style="border-top:2px green #B19292;">';
			$footerHTML .= '<tr><td width="20%" style="text-align:left; ">' . $celll . '</td><td width="60%" style="text-align:center;">' . $cellM . '</td><td width="20%" style="text-align:right;">Page ' . $this->getAliasNumPage() . '</td>';
			$footerHTML .= '</tr>';
			$footerHTML .= '</table>';
			...
			// $this->Cell(70, 10, 'TNG VT ARA v0622', 1, false, 'L', 0, '', 0, false, 'T', 'M');   
			// $this->Cell(40, 10, 'This is a copyright protected document. @ 2022', 0, false, 'C', 0, '', 0, false, 'T', 'M'); 
			// $this->Cell(70, 10, 'Page '.$this->getAliasNumPage(), 1, false, 'R', 0, '', 0, false, 'T', 'M');
		}
	}
