...
if(isset($_GET["Acct_ID"])){
	...
	// Check to see if we got a result here.  If we did not, then
	//  this is probably a sub location, so try to get the AR Number of the parent location.
	if (mysqli_num_rows($result) > 0) {
		...
		$query2 = "SELECT * FROM `Acct` a WHERE
		          a.`Acct_ID`=".$dealerID;
		$result2 = $link->query($query2);
		$row2 = $result2->fetch_assoc();


		$dealerName = $row2["Acct_Nm"];
		$dealerParentID = $row2["Prnt_Acct_ID"];
/*
echo "dealerID=".$dealerID;
echo "<br />dealerName=".$dealerName;
die();
*/
		if(is_numeric($dealerParentID)){
			// In this situation, we will set the dealerID to the parentID so other
			//  data is correctly fetched.
			//$dealerID = $dealerParentID;

			$query3 = "SELECT * FROM `Acct` a, Cntrct c, Cntrct_Dim cd WHERE
					  a.`Acct_ID`=".$dealerParentID." AND
					  a.Acct_ID = c.Mfr_Acct_ID AND
					  c.Cntrct_Dim_ID = cd.Cntrct_Dim_ID AND
					  cd.Cntrct_type_Cd is null;";
			...
			if (mysqli_num_rows($result3) > 0) {
				...
				$dealerARNumber = $row3["Assign_Rtlr_Nbr"];
			}else{
				// ERROR, fail back to start.
				header("location: pending_warranties.php");
				exit;
			}
		}else{
			// ERROR, fail back to start.
	...
	// If, after all that, we don't have a dealerAR Number, then fail
	if($dealerARNumber==""){
		header("location: pending_warranties.php");
		exit;
	}
...
/*
echo "Warranty_PDF=".$Warranty_PDF;
echo "<br />Warranty_PDF_SIGNED=".$Warranty_PDF_SIGNED;
die();
*/
...
die();
	...
	// Save the dealername that we determined above, so that it is not
	//  overwritten by the call to header.php
	//  The dealername may be that of a sub location which will get replaced by parent
	//  location name otherwise.
	$dealerNameLocal = $dealerName;
	...
	<div class="content-body">
		...
		<?php require_once("includes/common_page_content.php"); ?>
            ...
            <div class="col-md-6">
                ...
                <br />
                <b>Dealer Name:</b> <?php echo $dealerNameLocal; ?><br />
