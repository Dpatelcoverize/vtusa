		...
		$("#warrantyFormSubmit").click(function () {
			...
			if(($("input[type=radio][name=wrap_program]:checked").val() == "Y")){
				if( $("input:radio[name='isVehicleNew']:checked").val() == 'Y' && $("#grossWeight").val() == 'type 3' && $("input:radio[name='vehicleTierType']:checked").val() == 'B')
				{
					...
					Error_message += "<li>You have to select new Vehicle , Tier Type Battalion and Type 3 for Wrap program, and choose a correct term (2, 3, or 5 years) as well.</li>";
					flag51 = 0;
				}
			}
			else{
		...
		else
		{
			...
			$('input[type=radio][name=isVehicleNew]').change(function() {
				...
				if (this.value == 'N') {
					...
					if( $("input:radio[name='isVehicleNew']:checked").val() == 'Y' && $("#grossWeight").val() == 'type 3') {
						//$('input[name=coverageTerm]').attr("disabled",true);
