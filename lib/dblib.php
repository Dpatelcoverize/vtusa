...
function selectWarrantyBasePricing($link, $warrantyTerm, $warrantyType, $warrantyTier, $Wholesale_Flg = "N")
{
	...
	$query = "SELECT * FROM Wrnty_Std_Prcg WHERE Cvrg_Term_Yrs_Nbr=" . $warrantyTerm . " AND
	          ...
	          Base_Price_Table_Type_Code='" . $type_code . "';";
	$result = $link->query($query);
...
// Get Warranty base pricing for term, type and tier
function selectwrapWarrantyBasePricing($link, $warrantyTerm, $warrantyType, $warrantyTier, $Wholesale_Flg = "N")
{
	// Quick param check
	if (!is_numeric($warrantyTerm)) {
		return 0;
...
function selectAddlStdPrcgSum($link, $isAEP, $isAPU, $isAER, $isOld, $isEVBC, $isEEC, $isACP, $isHUDS, $isUCP, $isWearable = "")
{
	...
	// Overload this argument to either support 'Y'
	//  which is the original way to turn on or off the 'old' charge for a vehicle between 14-20 years old.
	//  Or the new 'OLD2' value which will tell it to look for the new 'old' option
	//  for a vehicle between 21-30 years old.
	if ($isOld == "OLD2") {
		if ($listOfItems != "") {
			$listOfItems .= ",";
		}
...
function createWarranty($link, $post)
{
	...
	if (!empty(trim($post["wearables"]))) {
		$wearable_Package = trim($post["wearables"]);
	}
	
	if (!empty(trim($post["quantity"]))) {
		$quantity = (int)trim($post["quantity"]);
	}
	...
	if (isset($_POST["wrntyStatDesc"]) && !empty(trim($_POST["wrntyStatDesc"]))) {
		$wholesale_flg = trim($_POST["wrntyStatDesc"]);
		$wrnty_Stat_Desc = trim($_POST["wrntyStatDesc"]);
	}else{
		$wholesale_flg = "";
	}
	...
	$sqlString .= "Vehicle_New_Flag,Vehicle_Description,Tier_Type,Apparatus_Equipment_Package,Aerial_Package,Electric_Vehicle_Battery_Coverage,Enhanced_Engine_Coverage,Ambulance_Conversion_Package,High_Use_Dept_Surcharges,Upfitter_Conversion_Package,";
	...
	mysqli_stmt_bind_param($stmt, "isssssssssssssissssssssisisssissssssssssissssssssssssssssss", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $val10, $val11, $val12, $val13, $val14, $val15, $val16, $val17, $val18, $val19, $val20, $val21, $val22, $val23, $val24, $val25, $val26, $val27, $val28, $val29, $val30, $val31, $val32, $val33, $val34, $val35, $val36, $val55, $val56, $val57, $val58, $val59, $val37, $val38, $val39, $val40, $val41, $val42, $val43, $val44, $val45, $val46, $val47, $val48, $val49, $val50, $val51, $val52, $val53, $val54);
	...
	if($wholesale_flg == "wholesale"){
		$wholesale_flg = "Y";
	}else if($wholesale_flg == "standard"){
		$wholesale_flg = "N";
	} else{
		...
		if ($result) {
			...
			$dealer_name = $account_row['Acct_Nm'];
			if ($account_row["Wholesale_Flg"] == "Y") {
				$wholesale_flg = $account_row["Wholesale_Flg"];
			} else {
				$wholesale_flg = "N";
			}
		} else {
	...
	$stmt = mysqli_prepare($link, "INSERT INTO Cntrct_Dim (Cntrct_type_cd,Cntrct_type_desc,Qte_Flg,Cstmr_Nme,
									   ...
									   Wrap_Flg,Wholesale_Flg,wearables_flag,Created_Date,EVBC_Flg,EEC_Flg,ACP_Flg,HUDS_Flg,UCP_Flg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?,?,?,?)");
	...
	$val29 = $customerPO;
	$val30 = $wrap_program;
	$val31 = $wearable_Package;

	mysqli_stmt_bind_param($stmt, "sssssssssssssdsssissssssissssssssssss", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $val10, $val11, $val12, $val13, $val14, $val15, $val16, $val17, $val18, $val19, $val20, $val21, $val22, $val23, $val24, $val25, $val26, $val27, $val28, $val29, $val30, $wholesale_flg, $val31, $val32, $val33, $val34, $val35, $val36);
	...
	$stmt = mysqli_prepare($link, "INSERT INTO Cntrct (Cntrct_Nbr,Cntrct_Sales_Chnl,Sply_Pkt_Shipd_Dte,
									   ...
									   Qte_Dt,Created_Date,Wrnty_Stat_Desc)
									   VALUES (?,?,?,?,?,?,?,?,?,NOW(),?)");
	...
	mysqli_stmt_bind_param($stmt, "sssiiiiiss", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $val10);
	...
	if ($wrap_program == 'N') {
		...
		$warrantyBasePricingResult = selectwrapWarrantyBasePricing($link, $termValue, $Vehicle_Type, $Tier_Type, $wholesale_flg);
	}
	...
	$stmt = mysqli_prepare($link, "UPDATE Cntrct SET Sales_Agt_Cost_Amt=?, Sales_Agt_Commission_Amt=?,
		                               Dlr_Cost_Amt=?, Dlr_Mrkp_Max_Amt=?, Dlr_Mrkp_Actl_Amt=?, MSRP_Amt=?, Additional_Commission_Amt=?, Quantity=? WHERE Cntrct_ID=?");
	...
	mysqli_stmt_bind_param($stmt, "iiiiiiiii", $val1, $val2, $val3, $val4, $val5, $val6, $val9, $val8, $val7);
	...
	if ($addlStdPrcgResult) {
		...
		$stmt = mysqli_prepare($link, "UPDATE Cntrct SET Addl_Sales_Agt_Cost_Amt=?, Addl_Sales_Agt_Commission_Amt=?,
										   ...
										   Addl_Dlr_Mrkp_Actl_AER_Amt=?, Addl_Dlr_Mrkp_Actl_WEARABLES_Amt=?, Addl_Dlr_Mrkp_Actl_EVBC_Amt=?, Addl_Dlr_Mrkp_Actl_EEC_Amt=?, Addl_Dlr_Mrkp_Actl_ACP_Amt=?, Addl_Dlr_Mrkp_Actl_UCP_Amt=? WHERE Cntrct_ID=?");

		$val1 = $Sales_Agt_Cost_Amt;
		$val2 = $Sales_Agt_Commission_Amt;
		$val3 = $Dlr_Cost_Amt;
		$val4 = $Dlr_Mrkp_Max_Amt;
		$val5 = $isHUDS == "Y" ? $Dlr_Mrkp_Actl_Amt - $Addl_Dlr_Mrkp_Actl_HUDS_Amt : $Dlr_Mrkp_Actl_Amt;
		$val6 = $isHUDS == "Y" ? $MSRP_Amt  - $Addl_Dlr_Mrkp_Actl_HUDS_Amt : $MSRP_Amt;
		$val7 = $Addl_Dlr_Mrkp_Actl_APU_Amt;
		$val8 = $Addl_Dlr_Mrkp_Actl_AEP_Amt;
		$val9 = $Addl_Dlr_Mrkp_Actl_AER_Amt;
		$val10 = $contract_ID;
		$val11 = $Addl_Dlr_Mrkp_Actl_EVBC_Amt;
		$val12 = $Addl_Dlr_Mrkp_Actl_EEC_Amt;
		$val13 = $Addl_Dlr_Mrkp_Actl_ACP_Amt;
		//$val14 = $Addl_Dlr_Mrkp_Actl_HUDS_Amt;
		$val15 = $Addl_Dlr_Mrkp_Actl_UCP_Amt;

		mysqli_stmt_bind_param($stmt, "iiiiiiiiiiiiiii", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $Addl_Dlr_Mrkp_Actl_WEARABLES_Amt, $val11, $val12, $val13, $val15, $val10);
	...
	if (isset($_SESSION["admin_id"])) {
		...
		$query = "SELECT emailAddress FROM Users WHERE userID = $id";
		$result = $link->query($query);
		$row = $result->fetch_assoc();
		$salesAdmin = $row['emailAddress'];
	}

	$Salesperson = isset($_SESSION["admin_username"]) ? $_SESSION["admin_username"] : "";

	$to = MAIL_SEND_ID;
	$isWarranty = $isQuote == 'Y' ? 'Quote' : 'Warranty';
	$subject = 'New ' . $isWarranty . ' Created';
	$txt = "New " . $isWarranty . " is Created by " . $dealer_name;
	$txt .= "
	<table style='border-collapse: collapse; width: 100%;'>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Dealer Name:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $dealer_name . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Salesperson:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $Salesperson . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Department Name:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>Department Name</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Dealer Id:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $_SESSION["id"] . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Warranty ID:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $contract_ID . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>VIN:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $Vehicle_Vin_Number . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Vehicle Type:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $Tier_Type . " - " . $Tier_Type_Desc . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Coverage Term:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $termValue . " years</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Warranty Date:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" .  date('Y-m-d', strtotime($agreementDateForInsert)) . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;' colspan='2'><b>Add on components</b></td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>AEP:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $Apparatus_Equipment_Package . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>APU:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $APU_Flg . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Aerial:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $Aerial_Package . "</td>
		</tr>
		<tr>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'><b>Wearables:</b></td>
			<td style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>" . $wearable_Package . "</td>
		</tr>
	</table>";
	$txt .= "\n\nThank you!\n<b>Vital Trends team</b>";

	$emailResult = sendEmail($to, '', '', $subject, $txt);
	$salesAdmin !== "" && $to != $salesAdmin && sendEmail($salesAdmin, '', '', $subject, $txt);
...
/**For converting any image to svg fromat */

function generateSVGFromImage($pngFilePath, $svgFilePath)
{
	// Escape file paths to prevent command injection
	$pngFilePath = escapeshellarg($pngFilePath);
	$svgFilePath = escapeshellarg($svgFilePath);

	// Determine the appropriate ImageMagick command based on the operating system
	if (PHP_OS === 'WINNT') {
		$command = "magick $pngFilePath $svgFilePath";
	} else {
		$command = "convert $pngFilePath $svgFilePath";
	}
