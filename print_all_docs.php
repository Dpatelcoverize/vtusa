...
//Wearable Addendum
$fileType = $_GET["isQuote"] == 'Y' ? 24 : 23;
$query = "SELECT Path_to_File FROM File_Assets WHERE Warranty_Cntrct_ID=" . $warrantyID . " AND File_Asset_Type_ID = $fileType ORDER BY File_Asset_ID DESC";
$result = $link->query($query);
$wearableAddendumResult = mysqli_fetch_assoc($result);
if ($wearableAddendumResult) {
	$hasWearableAddendum = "Y";
	$wearableAddendumPDF = substr($wearableAddendumResult["Path_to_File"], 1);
} else {
	$wearableAddendumPDF = "";
}
...
// Needs ECA if less than or equal to 2 years old and more than 500 miles
if ($isVhicleNew == "Y" && $odometerReadingMiles > 500 && ((date("Y") - $Vehicle_Year) <= 2)) {
	$needECA = "Y";
}
...
if ($hasSmallGoodsDetail == "Y") {
	if (file_exists($smallGoodsDetailPDF)) {
...
// if ($hasWearableAddendum == "Y") {
// 	$pdfFile5 =  strval($wearableAddendumPDF);
// 	$pdf->addPDF($pdfFile5, 'all', 'P');
// } else {
// 	array_push($remainingDocs, "Wearable Addendum not yet uploaded");
// }
...
if ($isVhicleNew == "Y") {
	...
	// 	// $pdf->addPDF('uploads/fillable_documents/EMV TNG VT ECA v0722 Fillable.pdf' , 'all', 'P');
	// 	// array_push($remainingDocs,"Needs ECA Form");

	// 	$ECAform = 'uploads/fillable_documents/EMV TNG VT ECA v0722 Fillable.pdf';
	// 	$filenameExt = basename($ECAform);
	// 	$RegularPDF = new GeneratePDF;
	// 	$emptyArray = [];
	// 	$pdfFile6 = $RegularPDF->convertToRegularPDF($emptyArray, $ECAform, $filenameExt); //method returning the path of regular generated pdf
