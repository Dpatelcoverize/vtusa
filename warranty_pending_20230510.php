...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if($edit_action=="Edit"){
		if($showQuotes=="Y"){
			header("location: create_warranty.php?isQuote=Y&warrantyID=".$warrantyID);
			exit;
		}else{
			header("location: create_warranty.php?warrantyID=".$warrantyID);
			exit;
		}

	}elseif($edit_action=="PriceSummary"){
		if($showQuotes=="Y"){
			header("location: warranty_summary.php?isQuote=Y&warrantyID=".$warrantyID);
		}else{
		...
		if($showQuotes=="Y"){
			header("location: warranty_print.php?isQuote=Y&warrantyID=".$warrantyID);
		}else{
			header("location: warranty_print.php?warrantyID=".$warrantyID);
			exit;
		}
		exit;
		...
		$stmt = mysqli_prepare($link, "INSERT INTO `Cntrct`
									   ...
									   Mfr_Acct_ID,Dlr_Cost_Amt,Sales_Agt_Cost_Amt,Sales_Agt_Commission_Amt,
									   ...
									   Addl_Dlr_Mrkp_Actl_AER_Amt,Created_Date)
		                               SELECT
       ...
       //Rewrite warranty PDF
		...
		$data = [
			...
			$pdf = new GeneratePDF;
			$pdf->generate($data , $pdfFileName , $type , $showQuotes);
...
if($userType=="dealer" && $isContactPerson!="Y" && $persID!=""){
	$limitView = "Y";
}else{
	$limitView = "N";
}
...
<?php
    ...
    ?>
	<td>
		<a href="small_goods_worksheet.php?warrantyID=<?php echo $row["Cntrct_ID"];?>">Edit</a> | <a href="<?php echo $smallGoodsPDF['Path_to_File']; ?>" target="__blank">Print</a> | <a href="<?php echo $smallGoodsDetailPDF['Path_to_File']; ?>" target="__blank">Detail</a><br>
		<a href="small_goods_summary.php?warrantyID=<?php echo $row["Cntrct_ID"];?>">Show Summary</a>
	</td>
<?php
	}
  }else{
