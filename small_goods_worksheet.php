...
if(isset($_GET["smallGoodsCoverageID"])){
	...
	$quantityCount = $row["Gnrc_Item_Cat_Qty_Cnt"];
...
//	die("here");
	...
	if(!empty(trim($_POST["quantityCount"]))){
        $quantityCount = trim($_POST["quantityCount"]);
    }
	...
	if($smallGoodsCoverageID!=""){
		...
		$sqlString  = "UPDATE Sml_Goods_Cvge SET Sml_Goods_Gnrc_Prcg_ID=?,item_cat_type_cd=?,item_cat_type_desc=?, Gnrc_Item_Cat_Qty_Cnt=?,";
		...
		$val4 = $quantityCount;
		$val5 = $manufacturedYear;
		$val6 = $manufacturerName;
		$val7 = $modelNumber;
		$val8 = $makeNumber;
		$val9 = $serialNumber;
		$val10 = $limitOfLiabilityAmount;
		$val11 = $haveReceipt;
		$val12 = $smallGoodsCoverageID;

		mysqli_stmt_bind_param($stmt, "ississsssssi", $val1,$val2,$val3,$val4,$val5,$val6,$val7,$val8,$val9,$val10,$val11,$val12);
		...
		$val9 = $serialNumber;
		$val10 = $limitOfLiabilityAmount;
		$val11 = $limitOfLiabilityAmount;
		$val12 = $agentCost;
		$val13 = $agentCommission;
		$val14 = $dealerCost;
		$val15 = $dealerMarkup;
		$val16 = $msrpTotal;
		$val17 = $haveReceipt;
		...
		mysqli_stmt_bind_param($stmt, "ississsssssiiiiisi", $val1,$val2,$val3,$val4,$val5,$val6,$val7,$val8,$val9,$val10,$val11,$val12,$val13,$val14,$val15,$val16,$val17,$val18);
...
*/
    ...
    *********************/
	...
	//removes 2 filters from above query because this query is used for both quote and warrenty
	$query = "SELECT * FROM Cntrct c, Cntrct_Dim cd, Veh v WHERE c.Cntrct_ID=".$warrantyID." AND
	c.Veh_ID = v.Veh_ID AND
	c.Cntrct_Dim_ID=cd.Cntrct_Dim_ID AND
	cd.Is_Deleted_Flg != 'Y' ";
    ...
    $data = [ 'CUSTOMER NAME' => $customerName,
		 ...
		 'QUANTITY SUBMITTED AA', 'QUANTITY APPROVED AA',
        ...
        while($row = mysqli_fetch_assoc($smallGoodSummary)) {
          ...
          if(isset($currentSelectionsArray[$row["Item_Cat_Type_Cd"]])){
			  ...
			  if($valuePlaceholder > 0)
			  {
				...
				else if($row["Item_Cat_Type_Cd"] == 'AA')
				{
					   $data['QUANTITY SUBMITTED AA'] = $valuePlaceholder;
					  // $data['QUANTITY APPROVED H'] = '$'.number_format(($row["MSRP_Amt"] * $valuePlaceholder),0);
					   $totalQuantitySubmitted += $valuePlaceholder;
				}
...
//Small goods detail PDF
			...
			for( $i=0 ; $i < $loopwillIterate ; $i++ ) {
				...
				$data = [ 'CUSTOMER NAME' => $customerName, 'Submission Date' => $agreementDate, 'TRUCK VIN' => $vin, 'Page #', 'Total Pages',
				 		' Entry 1 Type'  => "" , ' Entry 1 Purchase Date'  => "", 'Entry 1 Make'   => "", 'Entry 1 Model'  => "" , 'Entry 1 Serial #' => "", 'Entry 1 Receipt' => "YES", 'Entry 1 Approved' => "",
				 		' Entry 2 Type'  => "" , ' Entry 2 Purchase Date'   => "", 'Entry 2 Make'   => "", 'Entry 2 Model'  => "" , 'Entry 2 Serial #' => "", 'Entry 2 Receipt' => "Y", 'Entry 2 Approved' => "",
						' Entry 3 Type'  => "" , ' Entry 3 Purchase Date'   => "", 'Entry 3 Make'   => "", 'Entry 3 Model'  => "" , 'Entry 3 Serial #' => "", 'Entry 3 Receipt' => "yes", 'Entry 3 Approved' => "",
						' Entry 4 Type'  => "" , ' Entry 4 Purchase Date'   => "", 'Entry 4 Make'   => "", 'Entry 4 Model'  => "" , 'Entry 4 Serial #' => "", 'Entry 4 Receipt' => "y", 'Entry 4 Approved' => "",
...
?>
											...
											<div class="form-group col-md-6" id="quantityDiv1" style="display: none;">
												Quantity: 1
											</div>
											<div class="form-group col-md-6" id="quantityDiv2" style="display: none;">
												<label for="Quantity">Quantity:</label>
												<input type="number" class="form-control" max="40" min="1" id="quantityCount" name="quantityCount" value="<?php echo $quantityCount;?>" placeholder="Quantity" oninput="if(this.value < 1) this.value = 1; if(this.value > 40) this.value = 40;">
											</div>

											<script>
												document.addEventListener("DOMContentLoaded", function() {
													const dropdown = document.querySelector('select[name="smallGoodsPricingID"]');
													const quantityDiv1 = document.getElementById('quantityDiv1');
													const quantityDiv2 = document.getElementById('quantityDiv2');

													function toggleDivs() {
														if (dropdown.value === "18") {
															quantityDiv1.style.display = 'none';
															quantityDiv2.style.display = 'block';
														} else {
															document.getElementById('quantityCount').value = "1";
															quantityDiv1.style.display = 'block';
															quantityDiv2.style.display = 'none';
														}
													}
													dropdown.addEventListener('change', toggleDivs);
													toggleDivs()
												});
											</script>
                                            <div class="form-group col-md-6">
