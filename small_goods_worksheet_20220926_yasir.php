    ...
    *********************/
    ...
    if (mysqli_num_rows($smallGoodSummary) > 0) {
        ...
        while($row = mysqli_fetch_assoc($smallGoodSummary)) {
          ...
          $data['Estimated Items Cost'] =  '$'.$extendedTotal;
      }
...
$data = [ 'CUSTOMER NAME' => $customerName, 'Submission Date' => $agreementDate, 'TRUCK VIN' => $vin,
...
'Entry 30 Type', 'Entry 30 Purchase Date', 'Entry 30 Make', 'Entry 30 Model', 'Entry 30 Serial #', 'Entry 30 Receipt', 'Entry 30 Approved',
            ...
            while($row = mysqli_fetch_assoc($smallGoodsDetail)){
                $data['Entry '.$counter.' Type'] = $row["Item_Cat_Type_Cd"];
                $data['Entry '.$counter.' Make'] = $row["Mk_Nbr"];
                $data['Entry '.$counter.' Model'] = $row["Model_Nbr"];
                $data['Entry '.$counter.' Serial #'] = $row["Ser_nbr"];
                $counter++;
            }
...
$pdfFileName = str_replace(" ", "_", $customerName) . '_' . str_replace(" ", "_", $warrantyID) . '_' . time() . '.pdf';
        ...
        $pdf = new GeneratePDF;
	$response = $pdf->generateSgDetail($data, $pdfFileName);
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="basic-form dealer-form">
                                    ...
                                    <form name="dealerForm" method="POST" action="" enctype="multipart/form-data">
...
?>
                                            ...
                                            <div class="form-group col-md-6">
												<input type="text" class="form-control" name="manufacturedYear" value="<?php echo $manufacturedYear;?>" placeholder="Manufactured Year">
                                            </div>
