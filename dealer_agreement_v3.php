...
echo "dealerphone=".$dealerPhone;
	...
	if ($acctID != "") {
		...
		$stmt = mysqli_prepare($link, "INSERT INTO Acct (Acct_Nm,Fed_Tax_Number,EIN_Nbr,Duns_Number,Multiple_Locations,Individual_Billing,Sls_Agnt_ID,Dlr_Affiliate_Fee_Pct,Acct_Type_Cd,Acct_Type_Desc,Wholesale_Flg) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
		...
		mysqli_stmt_bind_param($stmt, "ssssssiisss", $val1, $val2, $val3, $val4, $val5, $val6, $val7, $val8, $val9, $val10, $val11);
	...
	if ($result) {
		...
		$moreInfo['user_type']='Primary Contact';
		$moreInfo['mail_type']='Dealer Agreement';
		$moreInfo['email_purpose']='welcome';	

		$emailResult = sendEmail($to, $primaryContactFirstName, $primaryContactLastName, $subject, $txt,$moreInfo);
		...
		if ($usePrimaryAsAP == "N" && $accountsPayableContactEmail != "") {
			...
			//mail($to,$subject,$txt,$headers);
			$moreInfo['user_type']='Accounts Payable Contact';
			$moreInfo['mail_type']='Dealer Agreement';
			$moreInfo['email_purpose']='welcome';

			$emailResult = sendEmail($to, $accountsPayableContactFirstName, $accountsPayableContactLastName, $subject, $txt,$moreInfo);
		}
	...
	$Data = [
		'Date' => $agreementDate, 'RETAILER BUSINESS NAME' => $dealerName, 'DOING BUSINESS AS' => $dba, 'FEDERAL TAX ID' => $federalTaxID, 'DUNS#' => $duns,
				...
				<div class="col-md-12">
					...
					?>
						...
						<div class="basic-form">
							...
							<form name="dealerLoginForm" action="dealer_login.php" method="POST" enctype='multipart/form-data'>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					...
					<div class="card-body">
						<div class="basic-form dealer-form row">
							<div class="watermark">
							...
							?>
								...
								?>
									<div class="form-group col-md-6">
										...
										<img class="signature-image" src="uploads/<?php echo $signatureFilename; ?>" />
									</div>
									<div class="form-group col-md-6">
							...
							?>
								...
								<form name="dealerForm" id="dealer_agreement_v3" method="POST" action="" enctype="multipart/form-data">
...
<script>
	jQuery('input').on('keypress', function(e) {
		...
		*/
			...
			if ($(".jSignature").jSignature("getData", "native").length == 0 && !jQuery('#signatureFile')[0].files[0]) {
