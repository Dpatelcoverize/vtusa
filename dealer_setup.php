...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty(trim($_POST["personEmail"]))) {
        ...
        if (filter_var($personEmail, FILTER_VALIDATE_EMAIL)) {
            ...
            $_SESSION["error_emessage"] = "Please provide email address";
        }
    }else{
        unset($_SESSION["error_emessage"]);
    }
    ...
    if (!empty(trim($_POST["personPhone"]))) {
        ...
        $personPhone = preg_replace('/[^0-9]/', '', trim($_POST["personPhone"]));
        // Define phone number length boundaries
        $minLength = 10;  // Minimum length, e.g., for local numbers
        $maxLength = 15;  // Maximum length, e.g., for international numbers
        // Check if the phone number is numeric and within the valid length range
        if (is_numeric($personPhone) && strlen($personPhone) >= $minLength && strlen($personPhone) <= $maxLength) {
            unset($_SESSION["error_pmessage"]);
        } else {
            $_SESSION["error_pmessage"] = "Invalid phone number. Please enter a valid number.";
        }
    ...
    if (isset($_SESSION["error_fmessage"]) != '' || isset($_SESSION["error_lmessage"]) != '' || isset($_SESSION["error_emessage"]) != '' || isset($_SESSION["error_pmessage"]) != '') { 
               header("location: dealer_setup.php");
        exit;
    }
    ...
    // This is the value for the Dealer Agent role type.
    $dealerAgentRoleID = 6;

    $stmt = mysqli_prepare($link, "INSERT INTO Users (Acct_ID,Pers_ID,Agent_ID,Role_ID,firstName,lastName,emailAddress,username,password,mustResetPassword,createdDate) VALUES (?,?,?,$dealerAgentRoleID,?,?,?,?,?,'Y',NOW())");

    /* Bind variables to parameters */
    $val1 = $dealerName;
    mysqli_stmt_bind_param($stmt, "iiisssss", $dealerID, $primary_Contact_Person_id, $primary_Contact_Person_id, $personFirstName, $personLastName, $personEmail, $personEmail, $initialPassword);
	...
	$emailResult = sendEmail($to, $personFirstName, $personLastName, $subject, $txt, $moreInfo);
        ...
        <div class="row" style="margin-top: 2%;">
            <div class="col-lg-12">
                <div class="form-group col-md-12">
                    <a href="dealer_addendum.php" class="btn btn-sm btn-secondary">
                                           Done Adding Users <i class="fa fa-angle-double-right"></i> Next
                                        </a>
                </div>
            </div>
        </div>
...
while ($row = mysqli_fetch_assoc($personResult)) {
    $loopCounter++;
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="email" class="form-control" name="personEmail" placeholder="" required value="<?php if (isset($_SESSION['personEmail']) != '') {
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text" class="form-control" name="personPhone" placeholder="" required value="<?php if (isset($_SESSION['personPhone']) != '') {
                                          ...
                                          <span style="color:red;<?php if (isset($_SESSION['error_pmessage']) != '') { ?>display:block; <?php
                                                                                                                                    } else { ?>display:none; <?php
                                                                                                                                                            } ?>"><?php if (isset($_SESSION['error_pmessage']) != '') {
                                                                                                                                                                        echo $_SESSION['error_pmessage'];
                                                                                                                                                                    } ?></span>                                                                                          
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>Notes</label>
...
ORDER BY Cntct_Prsn_For_Acct_Flg_Order,Acct_Nm, Pers_Last_Nm ASC;";
                                    ...
                                    if (mysqli_num_rows($personResult) > 0) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($personResult)) {
                                            $loopCounter++;
                                            $encryptedId = encryptData($row['Pers_ID']);
                                            $loc_id =  encryptData($row['Usr_Loc_ID']);
                                    ?>
                                            <tr>
                                                ...
                                                ?>
                                                <td><a href="manage_person.php?id=<?= $encryptedId ?>&loc=<?= $loc_id ?>">Edit</a></td>
                                                <?php } ?> 
                                            </tr>
                                        <?php
                                        }
